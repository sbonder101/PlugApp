...
<div class="grid grid-cols-1 gap-4 mt-5 border-t md:grid-cols-2 lg:grid-cols-3">
    ...
    @if(!empty($selectedModel->delivery_address))
                <x-details.item title="{{ __('Address') }}" text="">
                    <p class="text-sm font-semibold"><span class="text-sm font-bold">{{ __('Name') }}:</span> {{ $selectedModel->delivery_address->name ?? '' }}</p>
                    <p class="text-sm font-medium"><span class="text-sm font-bold">{{ __('Address') }}:</span> {{ $selectedModel->delivery_address->address ?? '' }}</p>
                    <p class="text-sm font-light"><span class="text-sm font-bold">{{ __('Description') }}:</span> {{ $selectedModel->delivery_address->description ?? '' }}</p>
                </x-details.item>
        @else
