<div @if ($ignore ?? false) wire:ignore @endif>
    ...
    <div class="relative inline-block w-full">
        <select class="block w-full p-0 border rounded appearance-none bg-grey-lighter text-grey-darker border-grey-lighter" {{-- on change --}} wire:change="{{ $onchange ?? '' }}" name="{{ $name ?? '' }}" @if ($multiple ?? false) multiple="multiple" @endif id="{{ $id ?? ($name ?? '') }}" @if ($defer ?? true) wire:model.defer='{{ $name ?? '' }}' @else wire:model='{{ $name ?? '' }}' @endif @if ($width ?? false) style="width: {{ $width }}%" @endif>
